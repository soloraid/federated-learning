...
def plot_simulate_report(n_simulation=10):
    ...
    scores_ax.plot(scores[n_simulation:n_simulation * 2:1], marker='o', linestyle='--', linewidth=3,
                   label='number of agents 2')
    scores_ax.plot(scores[n_simulation * 2:n_simulation * 3:1], marker='o', linestyle='--', linewidth=3,
                   label='number of agents 3')
    scores_ax.legend(loc='lower right')
    scores_ax.set_title('simulate scores in n_agents final models update_rate=')
    scores_ax.set_xlabel('iteration')
    scores_ax.set_ylabel('reward')
    scores_ax.set_facecolor('gray')
    scores_ax.grid(True)
    fig1.savefig('scores_update_rate.png')
    plt.show()
