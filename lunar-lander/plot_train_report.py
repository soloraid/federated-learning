...
def plot(n_agents):
    ...
    for index in range(1, n_agents + 1):
        agents_scores = pd.read_csv('results/' + str(index) + params.agents_scores_file, header=None)
        global_rewards = pd.read_csv('results/' + str(index) + params.global_rewards_file, header=None)
        agents_epsilons = pd.read_csv('results/' + str(index) + params.agents_epsilons_file, header=None)
        agents_scores.head()
        global_rewards.head()
